    ...
    <main className="mx-auto w-full max-w-3xl pb-10">
      ...
      {loading ? <div className="flex min-h-72 items-center justify-center"><Spinner /></div> : (
        <section className="mt-8">
          ...
          <p className="mt-2 text-sm leading-6 text-neutral-mid">Esta elección se aplica a tus fotos, bio, edad, ocupación, gustos y preferencias.</p>
          <div className="mt-7 grid gap-3">
            {OPTIONS.map((option) => {
              const active = value === option.value;
              return (
                <button key={option.value} type="button" disabled={saving} onClick={() => void select(option.value)} className={`flex min-h-28 items-center gap-4 rounded-card border p-5 text-left transition disabled:cursor-wait ${active ? "border-primary/35 bg-[#f0f6f2] ring-1 ring-primary/20" : "border-black/[0.07] bg-surface-raised hover:bg-[#f5f7f4]"}`}>
      ...
      <section className="mt-8">
        ...
        <div className="overflow-hidden rounded-card border border-black/[0.06] bg-surface-raised shadow-card">
          ...
          <div className="flex min-h-18 items-center gap-3 px-4 py-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-control bg-[#e9eeea] text-primary-dark"><LockKeyhole className="h-5 w-5" /></span>
