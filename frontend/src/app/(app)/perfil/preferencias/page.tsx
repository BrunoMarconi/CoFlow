  ...
  return (
    <div className="mx-auto max-w-3xl pb-24 sm:pb-10">
      <header className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1"><p className="text-3xs font-bold uppercase tracking-[0.15em] text-primary">Tu búsqueda</p><h1 className="font-rounded text-3xl font-semibold tracking-[-0.04em] text-brand-dark sm:text-4xl">Preferencias de vivienda</h1><p className="mt-2 max-w-lg text-sm leading-6 text-secondary">Ajusta lo que buscas para mejorar tus recomendaciones y comparaciones de convivencia.</p></div>
      </header>

      {loadFailed || !answers ? (
        <section className="mt-7 rounded-card border border-black/[0.06] bg-surface-raised p-6 text-center"><p className="text-base font-bold text-brand-dark">Completa primero tu perfil de convivencia</p><p className="mt-2 text-sm leading-6 text-muted">Necesitamos tus respuestas iniciales antes de poder editar estas preferencias.</p><button type="button" onClick={() => router.push("/onboarding")} className="mt-5 h-12 rounded-full bg-primary px-6 text-sm font-bold text-white">Completar cuestionario</button></section>
      ) : <>
        <section className="mt-7 rounded-card border border-black/[0.06] bg-surface-raised p-4 shadow-card sm:p-5">
          <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-control bg-[#e8eeea] text-primary-dark"><Banknote className="h-5 w-5" /></span><div><h2 className="text-sm font-bold text-brand-dark">Presupuesto mensual máximo</h2><p className="mt-0.5 text-xs text-secondary">Tu parte aproximada del alquiler.</p></div></div>
          ...
          <label className="mt-3 flex h-12 items-center rounded-control border border-black/[0.07] bg-white px-4 focus-within:border-primary/40 focus-within:ring-3 focus-within:ring-primary/8"><span className="text-sm font-bold text-primary-dark">€</span><input aria-label="Presupuesto mensual" type="number" inputMode="numeric" min={0} max={20000} value={budget} onChange={(event) => setBudgetOverride(event.target.value)} placeholder="Otra cantidad" className="h-full min-w-0 flex-1 bg-transparent px-3 outline-none" /><span className="text-xs text-muted">al mes</span></label>
        </section>

        <section className="mt-5"><div className="mb-2 flex items-end justify-between gap-3 px-1"><div><h2 className="font-rounded text-xl font-semibold tracking-[-0.02em] text-brand-dark">Convivencia</h2><p className="mt-1 text-xs text-secondary">Toca una preferencia para cambiarla.</p></div><span className="text-3xs font-bold uppercase tracking-[0.12em] text-muted">{preferenceFields.length} preferencias</span></div>
          <div className="divide-y divide-black/[0.055] overflow-hidden rounded-card border border-black/[0.06] bg-surface-raised shadow-card">{preferenceFields.map((field) => { const Icon = field.icon; return <button key={field.key} type="button" onClick={() => setActiveField(field)} className="flex min-h-18 w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-[#f4f7f4] sm:px-5"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-[#e8eeea] text-primary-dark"><Icon className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-brand-dark">{field.title}</span><span className="mt-0.5 block truncate text-xs text-secondary">{String(answers[field.key] || "Sin definir")}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-muted" /></button>; })}</div>
        </section>
