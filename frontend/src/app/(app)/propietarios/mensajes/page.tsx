...
export default function OwnerMessagesPage() {
  ...
  return (
    <div className="explore-shell -mx-6 -mt-4 flex min-h-[calc(100dvh-var(--mobile-header-height)-var(--safe-top))] w-[calc(100%+3rem)] flex-col px-6 pb-8 pt-5 sm:mx-auto sm:mt-0 sm:min-h-[calc(100dvh-var(--mobile-header-height)-3rem)] sm:w-full sm:max-w-7xl sm:rounded-sheet sm:p-7 lg:p-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-2xs font-bold uppercase tracking-[0.15em] text-primary">Panel de propietario</p><h1 className="mt-1 font-rounded text-3xl font-semibold tracking-[-0.045em] text-brand-dark sm:text-4xl">Mensajes</h1><p className="mt-2 text-sm leading-6 text-secondary">Conversaciones organizadas por vivienda.</p></div><Link href="/propietarios/solicitudes" className="inline-flex min-h-11 items-center gap-2 self-start rounded-full bg-surface px-4 text-sm font-bold text-brand-dark shadow-soft sm:self-auto"><Users className="h-4 w-4" /> Ver solicitudes</Link></header>
      ...
      <section className="mt-4 grid min-h-0 flex-1 overflow-hidden rounded-panel border border-black/[0.05] bg-surface shadow-soft lg:grid-cols-[minmax(20rem,.72fr)_1.28fr]">
