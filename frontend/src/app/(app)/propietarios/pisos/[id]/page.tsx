...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="rounded-[1.15rem] border border-[#dddddd] bg-white p-4"><span className="[&>svg]:h-5 [&>svg]:w-5">{icon}</span><strong className="mt-3 block text-lg">{value}</strong><span className="mt-1 block text-xs text-[#717171]">{label}</span></div>; }
