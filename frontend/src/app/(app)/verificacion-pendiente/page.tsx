...
export default function VerificacionPendientePage() {
  ...
  return (
    <div className="mx-auto flex min-h-[60vh] w-full max-w-lg flex-col items-center justify-center px-4 text-center">
      ...
      <p className="mt-5 text-3xs font-bold uppercase tracking-[0.16em] text-brand">Confirma tu correo</p>
      ...
      <p className="mt-6 border-t border-black/[0.06] pt-4 text-2xs leading-5 text-muted">Confirmar el correo demuestra que controlas esa dirección. No es una verificación documental de identidad.</p>
      </div>
    </div>
  );
}

function Step({ number, text }: { number: string; text: string }) { return <div className="rounded-control bg-white p-3"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#e8eeea] text-3xs font-bold text-primary-dark">{number}</span><p className="mt-2 text-3xs font-semibold leading-4 text-secondary">{text}</p></div>; }
