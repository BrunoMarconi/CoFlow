...
export default function LoginPage() {
  ...
  return <main className="min-h-dvh bg-surface px-5 py-[calc(var(--safe-top)+1.25rem)] sm:px-8 sm:py-8 lg:py-10"><div className="mx-auto w-full max-w-7xl"><AuthBrand /><div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(440px,0.95fr)] lg:items-start xl:gap-14">
    <section className="lg:pt-5"><h1 className="max-w-xl text-3xl font-bold leading-tight tracking-[-0.03em] text-brand-dark sm:text-5xl">Encuentra compañeros de piso que encajen contigo</h1><p className="mt-4 max-w-xl text-base leading-7 text-secondary sm:text-lg">Vive en comunidad y comparte más que un piso: comparte tu estilo de vida.</p><div className="mt-6 max-w-2xl"><AuthArtwork variant="login" /></div></section>
    <section className="mx-auto w-full max-w-xl rounded-24 border border-border bg-surface p-6 shadow-soft sm:p-9 lg:mt-8"><div className="text-center"><h2 className="text-2xl font-bold text-brand-dark sm:text-3xl">Inicia sesión</h2><p className="mt-2 text-sm text-secondary">Vuelve a tu comunidad CoFlow.</p></div>
