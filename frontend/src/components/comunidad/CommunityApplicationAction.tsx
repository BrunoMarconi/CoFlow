      ...
      <div className="sm:col-span-2">
        ...
        >
          ...
          {!reviewing ? <>
            ...
            {comparisons.length > 0 && <div className="mt-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold text-brand-dark">Hábitos comparables</p><p className="text-[11px] text-secondary">{matchingCount} de {comparisons.length} coinciden</p></div><div className="mt-2 grid gap-2 sm:grid-cols-2">{comparisons.map((item) => <div key={item.label} className="flex items-center gap-2 rounded-control bg-white px-3 py-2.5"><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[11px] font-bold ${item.matches ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{item.matches ? "✓" : "·"}</span><span className="min-w-0 flex-1"><span className="block text-[11px] font-bold text-brand-dark">{item.label}</span><span className="block truncate text-[10px] text-secondary">{item.matches ? item.communityValue : "Preferencias distintas · conviene hablarlo"}</span></span></div>)}</div><p className="mt-2 text-[10px] leading-4 text-muted">Las diferencias no significan incompatibilidad: son temas útiles para una primera conversación.</p></div>}
