  ...
  if (application?.status === "PENDING") {
    return (
      <div className="sm:col-span-2">
        <div className="flex min-h-14 flex-wrap items-center justify-between gap-3 rounded-card border border-primary/20 bg-[#eef5f1] px-4 py-3 sm:px-5">
    ...
    return (
      <div className="sm:col-span-2">
        <form
          ...
          className="rounded-card border border-black/[0.06] bg-surface-raised p-4 shadow-card sm:p-5"
        >
          <div className="flex items-start justify-between gap-3">
            <div><p className="text-3xs font-bold uppercase tracking-[0.14em] text-primary">{reviewing ? "Revisión final" : "Antes de solicitar"}</p><p className="mt-1 text-base font-bold text-brand-dark">{reviewing ? "Esto recibirá la comunidad" : `Tu encaje con ${community.name}`}</p></div>
            <span className="rounded-full bg-[#e9eeea] px-2.5 py-1 text-3xs font-bold text-primary-dark">{reviewing ? "Paso 2 de 2" : "Paso 1 de 2"}</span>
          </div>
          ...
          {!reviewing ? <>
            ...
            {budgetFits === false && <div className="mt-3 rounded-control border border-amber-200 bg-amber-50 px-3.5 py-3"><p className="text-xs font-bold text-amber-900">La aportación supera tu presupuesto actual</p><p className="mt-0.5 text-xs leading-5 text-amber-800">Puedes continuar si tu presupuesto es flexible, pero conviene explicarlo en tu presentación.</p></div>}

            {comparisons.length > 0 && <div className="mt-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold text-brand-dark">Hábitos comparables</p><p className="text-2xs text-secondary">{matchingCount} de {comparisons.length} coinciden</p></div><div className="mt-2 grid gap-2 sm:grid-cols-2">{comparisons.map((item) => <div key={item.label} className="flex items-center gap-2 rounded-control bg-white px-3 py-2.5"><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-2xs font-bold ${item.matches ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{item.matches ? "✓" : "·"}</span><span className="min-w-0 flex-1"><span className="block text-2xs font-bold text-brand-dark">{item.label}</span><span className="block truncate text-3xs text-secondary">{item.matches ? item.communityValue : "Preferencias distintas · conviene hablarlo"}</span></span></div>)}</div><p className="mt-2 text-3xs leading-4 text-muted">Las diferencias no significan incompatibilidad: son temas útiles para una primera conversación.</p></div>}

            <div className="mt-4 rounded-control bg-[#eef3ef] px-3.5 py-3"><p className="text-xs font-bold text-brand-dark">Una buena presentación suele incluir</p><p className="mt-1 text-xs leading-5 text-secondary">Tu situación actual, fecha aproximada para mudarte, rutina diaria y por qué te interesa esta comunidad.</p></div>
          </> : <div className="mt-4 rounded-card border border-black/[0.06] bg-white p-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-dark text-xs font-bold text-white">{[user?.first_name, user?.last_name].filter(Boolean).map((part) => part?.[0]).join("").slice(0, 2).toUpperCase() || "CF"}</span><span><span className="block text-sm font-bold text-brand-dark">{[user?.first_name, user?.last_name].filter(Boolean).join(" ") || "Tu perfil"}</span><span className="block text-xs text-secondary">{user?.rental_budget != null ? `Presupuesto: hasta ${user.rental_budget.toLocaleString("es-ES")} €/mes` : "Presupuesto no indicado"}</span></span></div><p className="mt-4 whitespace-pre-wrap rounded-control bg-[#f3f5f3] px-3.5 py-3 text-sm leading-6 text-secondary">{message.trim()}</p><p className="mt-3 text-3xs leading-4 text-muted">El administrador podrá abrir tu perfil para revisar tus datos públicos y preferencias de convivencia.</p></div>}
          ...
          {!reviewing && <textarea
            ...
            rows={4}
            disabled={submitting}
            className="mt-4 w-full resize-none rounded-field border border-black/[0.08] bg-white px-4 py-3 text-sm leading-6 text-brand-dark outline-none transition focus:border-primary/40 focus:ring-3 focus:ring-primary/8 disabled:opacity-60"
          />}
...
function SummaryMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[13px] bg-white px-3 py-2.5"><p className="text-3xs font-bold uppercase tracking-[0.1em] text-muted">{label}</p><p className="mt-1 text-xs font-bold text-brand-dark">{value}</p></div>;
}
