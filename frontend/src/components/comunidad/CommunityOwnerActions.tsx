...
export default function CommunityOwnerActions({ community }: { community: Community }) {
      ...
      <div className="rounded-2xl border border-hairline bg-white p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-neutral-strong">Cerrar comunidad</p>
            <p className="mt-1 text-xs leading-5 text-neutral-mid">Dejará de aparecer públicamente y sus miembros perderán acceso al espacio privado.</p>
          </div>
        </div>
  ...
  return (
    <div className="rounded-2xl border border-red-200 bg-white p-5">
      <h3 className="text-base font-semibold text-neutral-strong">¿Cerrar {community.name}?</h3>
      <p className="mt-2 text-sm leading-6 text-neutral-mid">Esta acción no se puede deshacer. Después podrás crear otra comunidad con tu cuenta.</p>
      ...
      <div className="mt-5 flex flex-col gap-2 sm:flex-row-reverse">
        ...
        <button type="button" onClick={() => { setConfirming(false); setError(""); }} disabled={deleting} className="flex min-h-11 flex-1 items-center justify-center rounded-xl border border-neutral-strong bg-white px-5 text-sm font-semibold text-neutral-strong hover:bg-[#f7f7f7] disabled:opacity-50">Cancelar</button>
      </div>
    </div>
  );
}
