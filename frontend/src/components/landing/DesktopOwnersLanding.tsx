    ...
    <div className="hidden bg-[#f3f8f6] text-brand-dark lg:block">
      <header className="sticky top-0 z-50 border-b border-[#dfe9e4] bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-[1440px] items-center gap-8 px-10 xl:px-14">
          ...
          <div className="flex items-center gap-3 text-[13px] font-medium"><Link className="px-3 py-2" href="/login">Iniciar sesión</Link><Link data-apple-magnetic className="apple-magnetic rounded-full bg-brand-mid px-5 py-3 text-white shadow-[0_8px_20px_rgba(49,95,75,.18)]" href="/register?role=owner">Publicar gratis</Link></div>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden border-b border-[#e3e7df] bg-[radial-gradient(circle_at_78%_16%,rgba(224,207,179,.38),transparent_34%),linear-gradient(180deg,#fffdfa_0%,#f5f4ee_100%)]">
        <div className="mx-auto grid min-h-[680px] max-w-[1440px] grid-cols-[.92fr_1.08fr] items-center gap-16 px-10 py-16 xl:px-14">
          ...
          <article data-apple-tilt className="apple-tilt ml-auto w-full max-w-[650px] overflow-hidden rounded-[38px] border border-white/80 bg-white/85 p-3 shadow-modal backdrop-blur">
            <div className="relative aspect-[1.45] overflow-hidden rounded-[29px]"><Image src="/images/owners-malaga-apartment-v2.png" alt="Salón luminoso de una vivienda en Málaga" fill priority sizes="650px" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-black/5" /><span className="absolute left-4 top-4 rounded-full bg-brand-dark/85 px-3 py-1.5 text-[11px] font-medium text-white backdrop-blur">Vivienda en Málaga</span><span className="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-medium backdrop-blur">Publicación gratuita</span><div className="absolute bottom-5 left-5 text-white"><p className="text-[11px] uppercase tracking-[.14em]">Más contexto desde el primer contacto</p><h2 className="mt-1 text-[28px] font-semibold tracking-[-.04em]">Decide a quién quieres responder</h2></div></div>
            <div className="p-4"><p className="text-[13px] leading-5 text-[#68736e]">Añade ubicación, capacidad, condiciones y plazas disponibles.</p><div className="mt-4 flex items-center justify-between gap-4"><div className="flex flex-wrap gap-2"><Chip>Publicación gratuita</Chip><Chip>Contacto directo</Chip><Chip>Solicitudes</Chip></div><Link className="flex shrink-0 items-center gap-1 rounded-full bg-brand-mid px-5 py-3 text-[13px] font-semibold text-white" href="/register?role=owner">Publicar ahora <ArrowRight size={13} /></Link></div></div>
          </article>
        </div>
      </section>
...
function InfoCard({ title, text }: { title: string; text: string }) { return <div className="rounded-card bg-[#f0f5f2] p-5"><p className="text-[13px] font-semibold">{title}</p><p className="mt-2 text-[12px] leading-5 text-[#6d7973]">{text}</p></div>; }
