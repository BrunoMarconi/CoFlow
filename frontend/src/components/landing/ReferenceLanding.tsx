      ...
      <div className="resident-desktop mx-auto max-w-[760px] px-4 lg:max-w-7xl lg:px-8">
        <section className="pb-9 pt-8">
          <div className="text-center">
            ...
            <Link href="/register" className="mx-auto mt-6 flex h-12 max-w-[420px] items-center justify-center rounded-full bg-brand-mid text-[14px] font-semibold text-white">Crear mi perfil gratis <ArrowRight className="ml-1.5" size={13} /></Link>
        ...
        <section className="compatibility-metric py-12">
          <div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {compatibilityAxes.map((axis) => <span key={axis} className="flex items-center gap-2 rounded-full bg-white px-3 py-2 text-[13px] font-medium text-brand-mid shadow-sm"><Check size={13} />{axis}</span>)}
            </div>
            <p className="mt-5 text-[13px] font-semibold text-brand-mid">CoFlow no decide por ti. Te ayuda a decidir con más información.</p>
          </div>
        ...
        <section className="balance-section py-12">
          ...
          <div className="mt-4 rounded-panel bg-white p-3 shadow-card">
            ...
            <div className="mt-3 space-y-2">{filterGroups[activeGroup].map(({ icon: Icon, title, text }, index) => <button type="button" key={title} onClick={() => setSelectedFilter(index)} aria-pressed={selectedFilter === index} className={`flex w-full items-center gap-3 rounded-control p-3 text-left transition ${selectedFilter === index ? "bg-[#edf3ef] ring-1 ring-brand-mid/15" : "bg-[#f5f6f6] hover:bg-[#f0f2f1]"}`}><span className="flex h-7 w-7 items-center justify-center rounded-full bg-white text-brand-mid"><Icon size={14} /></span><div className="min-w-0 flex-1"><p className="text-[14px] font-semibold">{title}</p><p className="mt-0.5 truncate text-[12px] text-[#808682]">{text}</p></div><span className={`h-4 w-4 rounded-full border transition ${selectedFilter === index ? "border-brand-mid bg-brand-mid shadow-[inset_0_0_0_4px_white]" : "border-black/10 bg-white"}`} /></button>)}</div>
            <div className="mt-3 flex items-center justify-between"><span className="flex items-center gap-1 text-[12px] text-[#75807a]"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />6 ejes de convivencia</span><Link href="/register" className="flex items-center gap-1 rounded-full bg-brand-mid px-3 py-2 text-[13px] font-semibold text-white">Crear mi perfil <ArrowRight size={11} /></Link></div>
          </div>
        </section>
        ...
        <section id="propietarios" className="resident-owner-section py-12">
          ...
          <div className="mt-4 rounded-panel bg-white p-3 shadow-card">
            <div className="relative aspect-[1.75] overflow-hidden rounded-card"><Image src="/images/owners-malaga-apartment-v2.png" alt="Salón luminoso de una vivienda en Málaga" fill sizes="(max-width: 768px) 100vw, 760px" className="object-cover" /><span className="absolute left-2.5 top-2.5 rounded-full bg-white/90 px-2.5 py-1 text-[12px] backdrop-blur">Málaga</span></div>
