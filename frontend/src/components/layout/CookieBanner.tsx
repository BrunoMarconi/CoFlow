  ...
  return <>
    ...
    <AnimatePresence>{visible && !panel && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-(--z-modal) flex items-end justify-center bg-black/20 p-3 backdrop-blur-[2px] sm:p-6">
      <motion.section initial={{ y: 28, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} role="dialog" aria-modal="true" aria-labelledby="cookie-title" aria-describedby="cookie-description" className="w-full max-w-[1040px] rounded-panel border border-black/8 bg-white p-5 shadow-modal sm:p-7 lg:p-8">
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="flex max-w-[690px] items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#eaf3ee] text-brand-mid"><CookieIcon /></span><div><h2 id="cookie-title" className="text-lg font-extrabold tracking-[-.02em] text-[#183126]">Tu privacidad, bajo tu control</h2><p id="cookie-description" className="mt-2 text-sm leading-6 text-[#64716b]">Usamos almacenamiento necesario para iniciar sesión y proteger la web. Actualmente no usamos analítica externa ni publicidad. Puedes aceptar, rechazar o configurar las preferencias opcionales.</p><p className="mt-2 text-xs text-[#76817c]">No activaremos tecnologías opcionales antes de tu elección. <Link href="/legal/cookies" className="font-bold text-brand-mid underline underline-offset-4">Política de cookies</Link></p></div></div>
    ...
    <AnimatePresence>{visible && panel && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[calc(var(--z-modal)+1)] grid place-items-center bg-black/35 p-4 backdrop-blur-sm">
      <motion.section initial={{ y: 20, opacity: 0, scale: .98 }} animate={{ y: 0, opacity: 1, scale: 1 }} role="dialog" aria-modal="true" aria-labelledby="cookie-settings-title" className="w-full max-w-xl rounded-panel bg-white p-5 shadow-2xl sm:p-7">
        <div className="flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#64716b]">Centro de privacidad</p><h2 id="cookie-settings-title" className="mt-2 text-2xl font-extrabold tracking-[-.035em] text-[#183126]">Configurar cookies</h2></div><button type="button" onClick={() => setPanel(false)} className="rounded-full bg-[#eff4f1] p-2.5 text-brand-mid" aria-label="Volver al aviso"><X size={17} /></button></div>
        ...
        <div className="mt-6 space-y-3"><Preference title="Necesarias" text="Sesión, seguridad y funciones solicitadas. No pueden desactivarse." checked disabled onChange={() => undefined} /><Preference title="Analítica" text="Eventos propios y minimizados para mejorar el embudo. Sin publicidad ni contenido de mensajes." checked={analytics} onChange={setAnalytics} /><Preference title="Preferencias opcionales" text="No utilizadas actualmente." checked={preferences} disabled onChange={setPreferences} /></div>
...
function ConsentButton({ children, onClick, secondary = false, className = "" }: { children: React.ReactNode; onClick: () => void; secondary?: boolean; className?: string }) { return <button type="button" onClick={onClick} className={`${className} min-h-11 rounded-full border border-brand-mid px-5 text-xs font-bold transition ${secondary ? "bg-white text-brand-mid hover:bg-[#f0f6f3]" : "bg-brand-mid text-white hover:bg-[#284e3e]"}`}>{children}</button>; }
function Preference({ title, text, checked, disabled = false, onChange }: { title: string; text: string; checked: boolean; disabled?: boolean; onChange: (value: boolean) => void }) { return <div className="flex items-center justify-between gap-5 rounded-2xl border border-black/7 bg-[#f7faf8] p-4"><div><h3 className="text-sm font-bold text-[#213b30]">{title}</h3><p className="mt-1 text-xs leading-5 text-[#6d7973]">{text}</p></div><button type="button" role="switch" aria-checked={checked} disabled={disabled} onClick={() => onChange(!checked)} className={`relative h-7 w-12 shrink-0 rounded-full transition ${checked ? "bg-brand-mid" : "bg-[#cbd4cf]"} disabled:cursor-not-allowed disabled:opacity-70`}><span className={`absolute top-1 flex h-5 w-5 items-center justify-center rounded-full bg-white shadow transition ${checked ? "left-6" : "left-1"}`}>{disabled && <Check size={11} className="text-brand-mid" />}</span></button></div>; }
