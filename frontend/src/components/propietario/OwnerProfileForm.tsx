...
export default function OwnerProfileForm({ mode = "create", initialValues, submitting = false, serverError = "", onSubmit, onCancel }: {
  ...
  return (
    <div className="explore-shell -mx-6 -mt-4 w-[calc(100%+3rem)] px-6 pb-12 pt-5 sm:mx-auto sm:mt-0 sm:w-full sm:max-w-4xl sm:rounded-sheet sm:p-7 lg:p-8">
      ...
      <section className="mt-6 overflow-hidden rounded-panel bg-surface p-2 shadow-soft sm:p-3">
      ...
      <section className="mt-4 flex items-start gap-3 rounded-card bg-mint-50 p-4 sm:p-5">
    ...
    <ViewportPortal>
      <div className="fixed inset-0 z-60 overflow-y-auto bg-surface-soft">
        <div className="mx-auto flex min-h-full w-full max-w-3xl flex-col px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(1.25rem,env(safe-area-inset-top))] sm:px-10">
          <header className="flex items-center justify-between gap-4"><button type="button" onClick={onBack} className="flex h-11 w-11 items-center justify-center rounded-full bg-surface text-brand-dark shadow-soft transition hover:bg-black/[0.035] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand" aria-label="Volver"><ChevronLeft className="h-5 w-5" /></button>{progress ? <span className="rounded-full bg-surface px-3 py-1.5 text-xs font-bold text-secondary shadow-soft">{progress}</span> : <span />}</header>
          ...
          <main className="flex flex-1 flex-col justify-center py-8 sm:py-12">
            ...
            {step === "owner_type" ? (
              <div className="mt-8 grid gap-3">
                {OWNER_TYPES.map((option) => <button key={option.value} type="button" onClick={() => setValue(option.value)} aria-pressed={value === option.value} className={`flex min-h-24 items-center gap-4 rounded-card border p-4 text-left transition sm:p-5 ${value === option.value ? "border-primary/30 bg-mint-50 ring-2 ring-primary/10" : "border-border hover:bg-surface-soft"}`}><span className={`flex h-11 w-11 items-center justify-center rounded-full [&>svg]:h-5 [&>svg]:w-5 ${value === option.value ? "bg-primary text-white" : "bg-surface-soft text-muted"}`}>{option.icon}</span><span className="flex-1"><span className="block font-bold text-brand-dark">{option.label}</span><span className="mt-1 block text-sm text-secondary">{option.description}</span></span>{value === option.value && <Check className="h-5 w-5 text-primary" />}</button>)}
              </div>
            ) : (
...
function OwnerRow({ icon, onClick, title, value }: { icon: React.ReactNode; onClick: () => void; title: string; value: string }) {
  return <button type="button" onClick={onClick} className="flex min-h-20 w-full items-center gap-3 rounded-card px-3 py-3 text-left transition hover:bg-surface-soft focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand sm:px-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-mint-50 text-primary [&>svg]:h-4.5 [&>svg]:w-4.5">{icon}</span><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-brand-dark">{title}</span><span className="mt-0.5 block truncate text-xs text-secondary sm:text-sm">{value}</span></span><ChevronRight className="h-4 w-4 text-muted" /></button>;
}
