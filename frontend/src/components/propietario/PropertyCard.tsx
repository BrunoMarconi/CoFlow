...
import { BedDouble, Eye, MapPin, Pencil, Users } from "lucide-react";
import PropertyStatusBadge from "./PropertyStatusBadge";
import { getCommunityErrorMessage } from "@/lib/communityErrors";
import type { PropertySummary } from "@/types/property";

export default function PropertyCard({ property, onPause, onResume, onMarkRented }: { property: PropertySummary; onMarkReady: (id: number) => Promise<void>; onPause: (id: number) => Promise<void>; onResume: (id: number) => Promise<void>; onMarkRented: (id: number) => Promise<void>; onArchive: (id: number) => Promise<void> }) {
  ...
  return (
    <article className="overflow-hidden rounded-[1.5rem] border border-[#dddddd] bg-white shadow-[0_7px_24px_rgba(0,0,0,0.055)] sm:grid sm:grid-cols-[15.5rem_1fr]">
      <Link href={`/propietarios/pisos/${property.id}`} className="relative block aspect-[16/10] overflow-hidden bg-[#f3f3f3] sm:aspect-auto sm:min-h-57">
      ...
      <div className="flex min-w-0 flex-col p-5 sm:p-6">
        ...
        {["READY", "PAUSED"].includes(property.status) ? <button type="button" disabled={actioning} onClick={() => run(onMarkRented)} className="sr-only">Marcar como alquilado</button> : null}
      </div>
    </article>
  );
}
