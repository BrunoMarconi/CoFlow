...
function EditStepScreen({ step, property, amenities, saving, error, onBack, onSave, onAddPhotos, onRemovePhoto, onMakeCover }: { step: EditStep; property: Property; amenities: Amenity[]; saving: boolean; error: string; onBack: () => void; onSave: (payload: PropertyUpdate) => Promise<void>; onAddPhotos: (files: FileList | null) => Promise<void>; onRemovePhoto: (id: number) => Promise<void>; onMakeCover: (id: number) => Promise<void> }) {
  ...
  if (step === "photos") {
    ...
    return <GuidedScreen title="Fotos de tu vivienda" subtitle="Elige la portada y añade imágenes que cuenten bien el espacio." onBack={onBack} onSave={onBack} saving={false} saveLabel="Listo" error={error}><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{sortedImages.map((image) => <div key={image.id} className="group relative aspect-square overflow-hidden rounded-card bg-surface-soft"><Image src={image.image_url} alt="" fill unoptimized sizes="33vw" className="object-cover" />{image.is_cover ? <span className="absolute left-2 top-2 rounded-full bg-brand-dark px-2.5 py-1 text-3xs font-bold text-white">Portada</span> : <button type="button" onClick={() => onMakeCover(image.id)} className="absolute left-2 top-2 min-h-9 rounded-full bg-white/95 px-3 text-3xs font-bold text-brand-dark shadow-soft">Hacer portada</button>}<button type="button" onClick={() => onRemovePhoto(image.id)} aria-label="Eliminar foto" className="absolute right-2 top-2 flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-red-600 shadow-soft"><Trash2 className="h-4 w-4" /></button></div>)}<label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-card border border-dashed border-primary/35 bg-mint-50 text-primary-dark"><ImagePlus className="h-7 w-7" /><span className="text-xs font-bold">Elegir fotos</span><input type="file" multiple accept="image/*" className="sr-only" onChange={(event) => onAddPhotos(event.target.files)} /></label><label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-card border border-dashed border-primary/35 bg-mint-50 text-primary-dark sm:hidden"><Camera className="h-7 w-7" /><span className="text-xs font-bold">Hacer una foto</span><input type="file" accept="image/*" capture="environment" className="sr-only" onChange={(event) => onAddPhotos(event.target.files)} /></label></div></GuidedScreen>;
  }

  return <GuidedScreen title={meta.title} subtitle={meta.subtitle} onBack={onBack} onSave={submit} saving={saving} error={error}>
    {step === "location" ? <AddressAutocomplete value={value} onChange={setValue} onResolved={(address) => { setResolvedAddress(address); setValue(address.addressLine); }} variant="inline" /> : step === "amenities" ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{amenities.map((amenity) => { const selected = selectedAmenities.includes(amenity.id); return <button key={amenity.id} type="button" aria-pressed={selected} onClick={() => setSelectedAmenities((current) => selected ? current.filter((id) => id !== amenity.id) : [...current, amenity.id])} className={`min-h-20 rounded-card border p-4 text-left text-sm font-bold transition ${selected ? "border-primary/30 bg-mint-50 text-primary-dark ring-2 ring-primary/10" : "border-border bg-surface text-brand-dark hover:bg-surface-soft"}`}>{amenity.label}{selected && <Check className="mt-3 h-4 w-4 text-primary" />}</button>; })}</div> : step === "description" ? <div><label htmlFor="property-description" className="sr-only">Descripción</label><textarea id="property-description" autoFocus value={value} onChange={(event) => setValue(event.target.value)} maxLength={2000} rows={7} className="w-full resize-none rounded-card border border-border bg-surface-soft p-4 text-base leading-7 text-brand-dark outline-none transition placeholder:text-muted focus:border-primary focus:ring-4 focus:ring-primary/10" placeholder={meta.placeholder} /><p className="mt-3 text-right text-xs font-semibold text-secondary">{value.length} / 2000</p></div> : <label className="flex items-baseline rounded-card border border-border bg-surface-soft px-5 py-4 focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/10"><span className="sr-only">{meta.title}</span><input autoFocus type={meta.numeric ? "number" : "text"} inputMode={meta.numeric ? "numeric" : "text"} min={meta.numeric ? 0 : undefined} maxLength={meta.numeric ? undefined : 150} value={value} onChange={(event) => setValue(event.target.value)} placeholder={meta.placeholder} className={`${meta.numeric ? "text-4xl sm:text-5xl" : "text-xl sm:text-2xl"} min-w-0 flex-1 bg-transparent font-rounded font-semibold tracking-[-0.035em] text-brand-dark outline-none placeholder:text-muted`} />{meta.suffix ? <span className="ml-3 text-xl font-semibold text-secondary">{meta.suffix}</span> : null}</label>}
  </GuidedScreen>;
}
